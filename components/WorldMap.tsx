...
export default function WorldMap({ tracks, onSelectCountry, selectedCountryCode }: WorldMapProps) {
  ...
  useEffect(() => {
    ...
    // Render floating overlays (hearts for loved countries, stars/bookmark for want-to-visit)
    const overlayGroup = g.selectAll('.country-marker')
      .data(pathCountries.filter((c: any) => {
        const track = trackMap.get(c.code3);
        return track && (track.isLoved || track.wantToVisit);
      }), (d: any) => d.code3)
      .join('g')
      .attr('class', 'country-marker pointer-events-none');
    ...
    overlayGroup.selectAll('text')
      .data((d: any) => {
        const track = trackMap.get(d.code3);
        const markers: string[] = [];
        if (track?.isLoved) markers.push('❤️');
        else if (track?.wantToVisit) markers.push('⭐');
        return markers;
      })
      .join('text')
